...
; is_pit(yes, AL) -> format('Lost: Fell into a pit!~n', []),
                   format('Score: ~p,~n Time: ~p', [S,T])
...
% score when agent is in the same cell as the wumpus
update_score(AL,_,AL) :-
update_score(-1000),
fail.

% score when agent is in the same cell as the gold
update_score(_,AL,AL) :-
update_score(1000).

% score when agent is in the same cell as the pit
update_score(AL,_,_) :-
is_pit(yes, AL),
update_score(-1000),
fail.
...
init_pits :-
    ...
    wumpus_location(WL),
    gold_location(GL),
    findall([X,Y], (between(1,S,X), between(1,S,Y), \+ [X,Y] = [1,1],\+ [X,Y] = GL, \+ [X,Y] = WL , random(0,100, R), R < 20), Pits),
    assert( pit_location(Pits) ).
...
init_gold :-
    ...
    world_size(S),
    wumpus_location(WL),
    findall([X,Y], (between(1,S,X), between(1,S,Y), \+ [X,Y] = [1,1], \+ [X,Y] = WL), Gold),
    random_member(GL, Gold),
    assert( gold_location(GL) ).
...
init_wumpus :-
    ...
    world_size(S),
    findall([X,Y], (between(1,S,X), between(1,S,Y), \+ [X,Y] = [1,1]), Wumpus),
    random_member(WL, Wumpus),
    assert( wumpus_location(WL) ).
...
gold_location(GL),
pit_location(PL),
format('Wumpus is in ~p,~n Gold is in ~p,~n Pits are in ~p~n', [WL,GL,PL]).
